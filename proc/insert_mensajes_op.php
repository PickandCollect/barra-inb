        ...
        if ($stmt_exp->execute()) {
            ...
            if ($result_exp->num_rows > 0) {
                ...
                $query_insert_comentario = "
                    INSERT INTO Comentarios (comentario, fecha_comentario, fk_usuario_emisor, fk_usuario_receptor, fk_expediente)
                    VALUES (?, NOW(), ?, ?, (SELECT id_exp FROM Expediente WHERE no_siniestro = ?))";

                if ($stmt_com = $conexion->prepare($query_insert_comentario)) {
                    // Vincular los parámetros (asignando los valores de usuario_emisor y usuario_receptor)
                    $stmt_com->bind_param("siis", $mensaje, $id_usuario_emisor, $id_usuario_receptor, $no_siniestro);
